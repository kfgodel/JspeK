  ...
  private void findUnbalancedPairs() {
    ...
    for (String letVariable : definedLetters) {
      if (!getters.containsKey(letVariable)) {
        throw new SpecException("Variable [" + letVariable + "] doesn't have a matching getter method in [" + validable.getSimpleName() + "]");
      }
    }
    Set<String> definedGetters = getters.keySet();
    for (String getVariable : definedGetters) {
      if (!letters.containsKey(getVariable)) {
        throw new SpecException("Variable [" + getVariable + "] doesn't have a matching setter method in [" + validable.getSimpleName() + "]");
      }
    }
  }
